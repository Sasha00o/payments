...
async def lifespan(app: FastAPI):
    ...
    submission_worker_task = asyncio.create_task(run_submission_worker(shutdown_event))
    metrics_worker_task = asyncio.create_task(run_metrics_worker())
    ...
    if submission_worker_task:
        submission_worker_task.cancel()
        with suppress(asyncio.CancelledError):
            await submission_worker_task
    
    if metrics_worker_task:
